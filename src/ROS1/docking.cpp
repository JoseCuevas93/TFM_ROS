//Docking ROS1 10 iteraciones  con movimiento por tiempos
...
// Declaración del publisher de cmd_vel
...
void reset_random() {
    ...
    // Generar valores aleatorios para la posición y orientación
    ...
    model_state.pose.orientation.z = random_orientation_z; // Orientació aleatòria
    model_state.pose.orientation.w = std::sqrt(1.0 - random_orientation_z * random_orientation_z); // Normalització de quaternion
    ...
    // Enviar la sol·licitud al servei
    ...
    if (client.call(srv) && srv.response.success) {
        ...
        ROS_ERROR("No s'ha pogut restaurar la posició del TurtleBot3.");
    }
}
...
void move_origin()
{
    ...
    // Función odomCallback definida dentro de move_origin
...
void odom_inicio(float& x, float& y, float& yaw) {
    ...
    if (file.is_open()) {
        file << "Iteración " << iteracion << ", los valores de odometría inicial son: "
             << "x=" << x << ", y=" << y << ", yaw=" << yaw << "\n";
...
void odom_final(float& x, float& y, float& yaw) {
    ...
    std::ofstream file(file_path, std::ios::app);
    if (file.is_open()) {
        file << "Iteración " << iteracion << ", los valores de odometría final son: "
             << "x=" << x << ", y=" << y << ", yaw=" << yaw << "\n";
