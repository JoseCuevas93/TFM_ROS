...
class Docking : public rclcpp::Node
{
  ...
  private:
   ...
   /* void mover_robot() {
    ...
    /*
    void mover_robot() {
    float K_angular = 0.5;  // Constante proporcional para giro
    float K_lineal = 0.5;   // Constante proporcional para avance
    float error, yaw_inicial, x_inicial, y_inicial;
    uint cnt;
    
    RCLCPP_INFO(this->get_logger(), "Inicio de movimiento");
    
    // Obtener odometría inicial
    yaw_inicial = yaw;
    x_inicial = x;
    y_inicial = y;

    // 1. Giro inicial con control proporcional
    cnt = 0;
    while (true) {
        float yaw_actual = yaw - yaw_inicial;
        error = giro_inicial - yaw_actual;
        twist_.angular.z = K_angular * error;
        twist_.linear.x = 0.0;
        pub_cmd_vel_->publish(twist_);
    ...
    // 2. Avance en línea recta con control proporcional
    cnt = 0;
    while (true) {
        float distancia_recorrida = sqrt(pow(x - x_inicial, 2) + pow(y - y_inicial, 2));
        error = avance - distancia_recorrida;
        twist_.linear.x = K_lineal * error;
        twist_.angular.z = 0.0;
        pub_cmd_vel_->publish(twist_);
    ...
    // 3. Giro final con control proporcional
    yaw_inicial = yaw;
    cnt = 0;
    while (true) {
        float yaw_actual = yaw - yaw_inicial;
    ...
    RCLCPP_INFO(this->get_logger(), "Finalización de DOCKING.");
}
    */
    void mover_robot() {
        // Constantes de velocidad
        float angular_velocity = 0.1;  // rad/s
        float linear_velocity = 0.05;  // m/s
        float Kpv = 1.0, Kpw = 1.0; // Ganancia del control proporcional
        float Kiv = 0.0, Kiw = 0.0; // Ganancia del control integral
        float w = 0, v = 0;
        float w_max = 0.5, v_max = 0.2;
    
        RCLCPP_INFO(this->get_logger(), "Pulsa botón para enviar Twist 1");
        getchar();
        rclcpp::sleep_for(std::chrono::milliseconds(500));
        ...
        while (!converged && rclcpp::ok()) {
            rclcpp::spin_some(this->get_node_base_interface());
            float yaw_actual = yaw - yaw_inicial;
            error = giro_inicial - yaw_actual;
            int_error = int_error + error;
            w = Kpw * error + Kiw * int_error;
            RCLCPP_INFO(this->get_logger(), "Error: %f, Int Error: %f, W: %f",error,int_error,w);
            if(w>w_max)
                w=w_max;
            else if(w<-w_max)
                w=-w_max;
        ...
        RCLCPP_INFO(this->get_logger(), "Pulsa botón para enviar Twist 2");
        getchar();
        rclcpp::sleep_for(std::chrono::milliseconds(500));
    
        // 2. Avanza en línea recta con control proporcional
        float x_inicial = x;
        float y_inicial = y;
        error = 0;
        int_error = 0;
        ...
        while (!converged && rclcpp::ok()) {
            rclcpp::spin_some(this->get_node_base_interface());
            float dist_actual = sqrt(pow(x - x_inicial, 2) + pow(y - y_inicial, 2));
            error = avance - dist_actual;
            int_error = int_error + error;
            v = Kpv * error + Kiv * int_error;
            RCLCPP_INFO(this->get_logger(), "Error: %f, Int Error: %f, V: %f",error,int_error,v);
            if(v>v_max)
                v=v_max;
            else if(v<-v_max)
                v=-v_max;
        ...
        RCLCPP_INFO(this->get_logger(), "Pulsa botón para enviar Twist 3");
        getchar();
        rclcpp::sleep_for(std::chrono::milliseconds(1000));
        ...
        twist_.angular.z = 0.0;
        pub_cmd_vel_->publish(twist_);
        RCLCPP_INFO(this->get_logger(), "Finalización DOCKING.");
    }
