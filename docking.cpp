...
// Variables para almacenar los valores de odometrÃ­a
...
// DeclaraciÃ³n del publisher de cmd_vel
...
void reset_random() {
    ...
    // Generar valores aleatorios para la posiciÃ³n y orientaciÃ³n
    float random_x = 2.0 + static_cast<float>(std::rand()) / (static_cast<float>(RAND_MAX / (3.0 - 2.0)));
    float random_y = -1.0 + static_cast<float>(std::rand()) / (static_cast<float>(RAND_MAX / (1.0 - (-1.0))));
    float random_orientation_z = static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX);

    // Esperar fins que el servei estigui disponible
    ros::service::waitForService("/gazebo/set_model_state");

    // Crear un client per al servei
    ros::ServiceClient client = nh.serviceClient<gazebo_msgs::SetModelState>("/gazebo/set_model_state");

    // Configurar l'estat del model
    gazebo_msgs::ModelState model_state;
    model_state.model_name = "turtlebot3_waffle"; // Nom correcte del model
    model_state.pose.position.x = random_x;
    model_state.pose.position.y = random_y;
    model_state.pose.position.z = 0.0;
    model_state.pose.orientation.x = 0.0;
    model_state.pose.orientation.y = 0.0;
    model_state.pose.orientation.z = random_orientation_z; // OrientaciÃ³ aleatÃ²ria
    model_state.pose.orientation.w = std::sqrt(1.0 - random_orientation_z * random_orientation_z); // NormalitzaciÃ³ de quaternion
    ...
    // Enviar la solÂ·licitud al servei
    gazebo_msgs::SetModelState srv;
    srv.request.model_state = model_state;

    ros::Duration(1).sleep(); // Espera 1 segundo

    if (client.call(srv) && srv.response.success) {
        ROS_INFO("Posicio restaurada a (%.2f, %.2f, 0) amb orientacio %.2f.", random_x, random_y, random_orientation_z);
    } else {
        ROS_ERROR("No s'ha pogut restaurar la posiciÃ³ del TurtleBot3.");
    }
}
...
void move_origin()
{
    ...
    ROS_INFO("Comenzando movimiento a Origen");
    bool goal_reached = false;
    bool final_turn_completed = false;
    bool move_to_origin_done = false;
    
    
    // FunciÃ³n odomCallback definida dentro de move_origin
   // auto odomCallback = [&cmd_vel_pub](const nav_msgs::Odometry::ConstPtr& msg) {
        //static bool move_to_origin_done = false; // Para indicar si la tarea se ha completado
        ros::Subscriber odom_sub = nh.subscribe<nav_msgs::Odometry>("/odom", 10, [&](const nav_msgs::Odometry::ConstPtr& msg) {
        if (move_to_origin_done)
        {
        ...
        // Fase 1: Aproximarse al punto (0, 0)
        //static bool goal_reached = false;
        //static bool final_turn_completed = false;
        //ROS_INFO("Comenzando movimiento a Origen");
        ...
        if (!goal_reached)
        {
            ...
            {
                goal_reached = true;
                ROS_INFO("Llegado al Origen (0, 0).");
                ros::Duration(0.5).sleep(); // Espera 0.5 segundos
          
            }
        }
        else if (!final_turn_completed)
        {
            ...
            else
            {
                ...
                cmd_vel.linear.x = 0.0;
                cmd_vel.angular.z = 0.0;
                ROS_INFO("Orientacion completada.");
                cmd_vel_pub.publish(cmd_vel); // Detener el robot
                //ros::Duration(0.5).sleep(); // Espera 0.5 segundos
                move_to_origin_done = true;
                //ros::shutdown(); // Detener el nodo si es necesario
            }
        }
    ...
    // Usar un bucle con ros::spinOnce() para no bloquear el hilo principal
    ros::Rate rate(10); // Frecuencia de ejecuciÃ³n (10 Hz)
    while (ros::ok() && !move_to_origin_done)
    {
        ros::spinOnce(); // Procesa callbacks
        rate.sleep();    // Mantiene la frecuencia
    }

    ROS_INFO("Movimiento a origen completado.");
    return;
}

// FunciÃ³n de callback global
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
    ...
    // Encontrar el valor mÃ¡s pequeÃ±o entre los primeros 180 valores
    ...
    if (!output_file_txt.is_open() || !output_file_csv.is_open()) {
        ...
        return;  // Eliminas el ros::shutdown() para seguir con la ejecuciÃ³n
    }
...
x_extrem_izq = x_coords.back(); // Coordenada x del extrem esquerre de la placa
        ...
        for (auto it = valid_ranges_right.rbegin(); it != valid_ranges_right.rend(); ++it) {
            ...
            output_file_txt << it->first << " " << it->second << " " << x << " " << y << "\n"; // Angle, distÃ ncia i coordenades (x, y)

            // Escribir al archivo CSV
            output_file_csv << it->first << ", " << it->second << ", " << x << ", " << y << "\n"; // Angle, distÃ ncia, coordenades (x, y)
        }

        x_extrem_der = x_coords.back(); // Coordenada x del extrem dret de la placa

        // Guardar el primer valor de la derecha y el Ãºltimo valor de la izquierda en variables globales
        ...
        if (!valid_ranges_left.empty()) {
            angulos_izq = valid_ranges_left.back().first; // Ãšltimo valor de la izquierda
        }

        // Calcular la pendiente (m) y la intersecciÃ³n (b) del segmento utilizando la fÃ³rmula
        int n = x_coords.size();
        if (n > 1) {
	    ...
	    //CÃ lculs TrigonomÃ¨trics (dist_to_placa) fabs(numero);
	    ...
	    if (Pdx >= 0){ //Pd esta en el quadrant 1
	    	...
	    	giro_inicial = - (1.5708  - alfa); //Gir sentit horari, el valor ha de ser negatiu -(90Âº - alfa)
	    	
	    		if (m >= 0){ //Pendent placa positiva
	    
	    			giro_final = M_PI  - alfa - tecta;	//180Âº - alfa - tecta
	    ...
	    if (Pdx < 0){	//Pd esta en el quadrant 2
	    	...
	    	giro_inicial = M_PI_2  - alfa; 	//Gir sentit antihorari (positiu (90Âº - alfa))
	...
	output_file_txt << "Resultats cÃ lculs trigonomÃ¨trics:\n";
	...
	output_file_txt << "M_PI: " << M_PI << "\n";  
            ...
            output_file_csv << "\nPendents (m), IntersecciÃ³ (b), DistÃ ncia entre extrems (m), x_extrem_izq, y_extrem_izq, x_extrem_der, y_extrem_der\n";
...
void mover_robot() {
    ...
    // Constantes de velocidad (ajustables segÃºn el robot)
    ...
    ros::Duration(giro_time).sleep(); // MantÃ©n el giro durante el tiempo necesario
    ...
    // 2. Avanza en lÃ­nea recta lo que valga avance
    twist.linear.x = (avance < 0 ? -linear_velocity : linear_velocity);
    twist.angular.z = 0.0;  // AsegÃºrate de que no haya rotaciÃ³n mientras avanza
    cmd_vel_pub.publish(twist);

    // Calcula el tiempo necesario para avanzar
    double avance_time = fabs(avance) / linear_velocity;
    ros::Duration(avance_time).sleep(); // MantÃ©n el avance durante el tiempo necesario
    ...
    ros::Duration(giro_final_time).sleep(); // MantÃ©n el giro durante el tiempo necesario
...
// FunciÃ³n para convertir un cuaterniÃ³n a yaw
double getYawFromQuaternion(const geometry_msgs::Quaternion& q) {
    tf::Quaternion quat(q.x, q.y, q.z, q.w);
    double roll, pitch, yaw;
    tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);  // Convierte cuaterniÃ³n a Ã¡ngulos de Euler
    return yaw;  // Devuelve el yaw (rotaciÃ³n alrededor del eje Z)
}

// FunciÃ³n para calcular la odometrÃ­a
void odom_inicio(float& x, float& y, float& yaw) {

    ros::NodeHandle nh;
    bool is_processed = false;

    // Callback que se ejecutarÃ¡ al recibir el mensaje
    auto odomCallback = [&x, &y, &yaw, &is_processed](const nav_msgs::Odometry::ConstPtr& msg) {
        if (!is_processed) {
            x = msg->pose.pose.position.x;
            y = msg->pose.pose.position.y;

            // Calcular el yaw a partir del cuaterniÃ³n de orientaciÃ³n
    ...
    // Crear el suscriptor al tÃ³pico de odometrÃ­a
    ros::Subscriber odom_sub = nh.subscribe<nav_msgs::Odometry>("odom", 10, odomCallback);
    ...
    // Abrir el archivo en modo de aÃ±adir datos
    std::ofstream file(file_path, std::ios::app);
    if (file.is_open()) {
        file << "IteraciÃ³n " << iteracion << ", los valores de odometrÃ­a inicial son: "
             << "x=" << x << ", y=" << y << ", yaw=" << yaw << "\n";
...
void odom_final(float& x, float& y, float& yaw) {
    ...
    std::ofstream file(file_path, std::ios::app);
    if (file.is_open()) {
        file << "IteraciÃ³n " << iteracion << ", los valores de odometrÃ­a final son: "
             << "x=" << x << ", y=" << y << ", yaw=" << yaw << "\n";
...
// FunciÃ³n para convertir un string HH:MM:SS a segundos desde las 00:00:00
int convertirATiempoEnSegundos(const std::string& tiempo) {
    std::tm t = {};
    std::istringstream ss(tiempo);
    ss >> std::get_time(&t, "%H:%M:%S");
    if (ss.fail()) {
        throw std::invalid_argument("Formato de tiempo no vÃ¡lido: " + tiempo);
    }
    return t.tm_hour * 3600 + t.tm_min * 60 + t.tm_sec;
}

// FunciÃ³n para convertir radianes a grados
float radiansToDegrees(float radians) {
    return radians * (180.0 / M_PI);
}

// FunciÃ³n que calcula y escribe los resultados
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        // Resetear posiciÃ³n aleatoria
        ROS_INFO("Resetear posicion aleatoria");
        reset_random();

        ros::Duration(0.5).sleep();

        // Mover al origen
        ROS_INFO("Mover al origen");
        move_origin();

        // Calcular odometrÃ­a inicial
        ROS_INFO("Calcular odometrÃ­a inicial");
        ...
        ROS_INFO("Final de la iteracion %d", i + 1);
        iteracion++;
    }
